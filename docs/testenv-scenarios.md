...
| ID | Peers | Actions | Expected result |
|---|---|---|---|
| restart1 | A,B,C,D | Invite-only channel with B, C admitted; **A restarts**; D is invited | ✅ **Fixed.** Was: A held `[B, C, D]` while B and C stayed at `[A, B, C]` and B's send never reached D. Now all four views agree, B learns about D in 1.0s. Kept as a regression guard, restaged on the member-list version once the subscriber list it originally covered was removed |
...
| restart5 | A,B,C | A single tester is reset (data wiped, same slot) | ✅ Returns as a new identity holding nothing, and the owner keeps a member row for an identity that will never reappear |
...
| Finding | Detail |
|---|---|
| **A fake transport's delivery threads outlived their test.** `FakeRRCTransport` delivers on a thread, and the adversarial RRC cases built seven of them without draining any. The leftovers kept waking during later tests and were enough on their own to fail a timing-sensitive one elsewhere, which read as two different intermittent sync failures rather than as one cause | Fixed: the adversarial class drains them in an autouse fixture, the way `test_rrc_client.py` and `test_rrc_hub.py` already did. Two consecutive full-suite runs clean at 2193 afterwards, from one failure in each of the two before. The same trap applies to `fake_node.py` and `fake_file_transport.py`, so the rule is now in CLAUDE.md |
...
| **restart1**, `_subscriber_versions` lived only in memory, so a restarted owner renumbered from 1 and every later list was rejected as a replay | Fixed on `main` by the August security audit, which persisted the counter and re-checked the version under the commit lock. Both the subscriber list and its version table were removed with public channels; the same replay rule still governs member-list documents, which is what restart1 now covers |
