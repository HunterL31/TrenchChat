...
- **More than one message can carry** is flagged with `F_SYNC_TRUNCATED` on the response, whether that is rows past `MAX_RESPONSE_MESSAGES` or descriptions past the byte budget. The requester then asks again with a fresh summary of what it now holds, which the rows just received have changed, so the next answer carries the next batch. A difference the requester itself could not fit into a continuation is remembered per (channel, peer) and asked about the same way once the narrowing in flight has run its course; without that, a responder answering the part it did receive with rows and no ranges would end the exchange with the rest still missing, which is how both peers came to report synced while each lacked rows in the long-absence tests. That debt is paid whether or not the answer that ended the narrowing moved anything: two peers reconciling each other at once fill each other's ranges, so the last answer of an exchange routinely carries nothing new while a run of history is still missing from both sides, and in a two-peer channel there is no third party whose later answer could pay it. The single answer that ends the exchange instead is one naming an `F_SYNC_SCAN_CURSOR` no later than the point already asked from: only the sweep path sends that field, and it says outright that the responder swept no further, so the same question would return the same batch. `MAX_SYNC_CONTINUATIONS` bounds a peer that reports truncation forever.
