...
**A mention is deliberately left alone.** It is the mentioned identity's hash
written into the words as `@<hex>` (`protocol.mentions_in`), and there is no
name beside it to fall back to: the reader resolves one from what it knows the
identity as now. Rewriting it would mean sending a display name we cannot vouch
for in place of the address that was written, and a conversation has only one
other person in it to ping anyway, so the compose bar offers no mention picker
there. A hash somebody typed by hand travels as the hash they typed.
