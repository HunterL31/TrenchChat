...
- Closing the client window does not stop the node: the launcher keeps the backend running
  behind a tray icon (`trenchchat/tray.py`, optional — a machine with no tray gets None and
  the old quit-with-the-window behaviour) and reopens the window from there. That makes a
