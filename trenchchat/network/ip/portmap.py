"""
Asking the router for an inbound port, with nothing but the standard library.

A node behind a NAT can sometimes ask the router to forward a port to it, which
becomes the `mapped` candidate in an upgrade offer and the one address a peer
can reach it at without a punch. Two protocols cover almost every home router:
UPnP-IGD (SSDP discovery, then SOAP over HTTP) and NAT-PMP (a twelve-byte UDP
request to the default gateway).

Every part of this is best effort. No gateway, no answer, or a refusal all mean
the node simply has no mapped candidate and the punch has to work without one,
so nothing here raises past PortMapper: a failure is logged once and never
again, because a node on a router that will not map must not spend a log line
on saying so every minute.

**This has never run against real router hardware.** The wire encoders and
parsers are unit-tested in tests/test_portmap.py; discovery, the SOAP round
trip and the NAT-PMP round trip are untested against anything, and remain the
largest open item the Phase 0 spike left behind.

Every call here blocks on network I/O with its own timeout, up to about six
seconds when nothing answers, so it belongs on a worker thread and never on an
RNS callback thread.
"""
...
def add_mapping(port: int, protocol: str = "UDP",
                lifetime: int = DEFAULT_LEASE_SECS) -> dict:
    """Ask for a mapping by whichever protocol answers, NAT-PMP first.

    NAT-PMP is one datagram to a known address and fails in a fraction of a
    second where nothing speaks it; UPnP costs a multicast search and an HTTP
    fetch, so it is the second ask rather than the first.
    """
    try:
        return natpmp_add_mapping(port, protocol, lifetime)
    except PortMapError as natpmp_error:
        try:
            return upnp_add_mapping(port, protocol, lifetime)
        except PortMapError as upnp_error:
            raise PortMapError(f"NAT-PMP: {natpmp_error}; UPnP: {upnp_error}") from None


def delete_mapping(method: str, port: int, protocol: str = "UDP") -> dict:
    """Remove a mapping this node asked for, by the protocol that made it."""
    if method == "natpmp":
        return natpmp_delete_mapping(port, protocol)
    return upnp_delete_mapping(port, protocol)


# How much of a lease may pass before it is renewed. A router that forgets a
# mapping early costs the node its one reachable candidate, so half is early
# enough to survive one missed renewal.
RENEW_FRACTION = 0.5

# How long a node that could not get a mapping waits before asking again, and
# the ceiling that wait doubles to. A router with no UPnP will never grow one,
# so the cost of asking settles at once an hour.
RETRY_SECS = 300.0
MAX_RETRY_SECS = 3600.0


@dataclass(frozen=True)
class PortMapping:
    """One inbound port a router is currently forwarding to this node."""
    ...
    def due_at(self) -> float:
        """When this mapping has to be renewed to stay up."""
        return self.obtained_at + self.lifetime_secs * RENEW_FRACTION


class PortMapper:
    """This node's mapped candidate: obtained at startup, renewed before it expires.

    refresh() is the whole interface: call it on a worker thread as often as
    is convenient and it decides whether there is anything to do. A node with
    no gateway spends one failed attempt, logs it once at debug, and then asks
    again only on the retry schedule.
    """

    def __init__(self, port: int, *, protocol: str = "UDP",
                 lifetime_secs: int = DEFAULT_LEASE_SECS):
        self._port = port
    ...
    @property
    def mapping(self) -> PortMapping | None:
        """The mapping in hand, or None while this node has no mapped candidate."""
        return self._mapping

    def address(self) -> tuple[str, int] | None:
        """The mapped candidate as an offer carries it, or None."""
        if self._mapping is None:
            return None
        return self._mapping.host, self._mapping.port

    def refresh(self, now: float | None = None) -> PortMapping | None:
        """Obtain or renew the mapping if it is time to. Never raises."""
        now = time.time() if now is None else now
        if self._port <= 0:
            return None
        if self._mapping is not None and now < self._mapping.due_at():
            return self._mapping
        if self._mapping is None and now < self._next_attempt:
            return None
        try:
            result = add_mapping(self._port, self._protocol, self._lifetime)
        except PortMapError as e:
            self._mapping = None
            self._retry_secs = min(self._retry_secs * 2, MAX_RETRY_SECS)
            self._next_attempt = now + self._retry_secs
            if not self._reported:
                self._reported = True
                RNS.log(f"TrenchChat [ip]: no router mapping for port "
                        f"{self._port}: {e}", RNS.LOG_DEBUG)
            return None
        host = result.get("external_address") or ""
        external_port = int(result.get("external_port") or self._port)
        lifetime = int(result.get("lifetime") or self._lifetime)
        if not host:
            self._next_attempt = now + self._retry_secs
            return None
        first = self._mapping is None
        self._mapping = PortMapping(method=result.get("method", ""), host=host,
                                    port=external_port, lifetime_secs=lifetime,
                                    obtained_at=now)
        self._retry_secs = RETRY_SECS
        if first:
            RNS.log(f"TrenchChat [ip]: router mapped {host}:{external_port} to "
                    f"port {self._port} by {self._mapping.method}", RNS.LOG_NOTICE)
        return self._mapping

    def release(self) -> None:
        """Give the mapping back, so the router is not left holding it."""
        mapping = self._mapping
        self._mapping = None
        if mapping is None:
            return
        try:
            delete_mapping(mapping.method, self._port, self._protocol)
        except PortMapError as e:
            RNS.log(f"TrenchChat [ip]: could not release the mapping for port "
                    f"{self._port}: {e}", RNS.LOG_DEBUG)
